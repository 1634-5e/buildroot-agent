...
class TestConnectionManager:
    ...
    async def test_add_device(self, manager):
        ...
        await manager.add_device(device_id, mock_connection, "websocket")
    ...
    @pytest.mark.asyncio
    async def test_add_device_socket(self, manager):
        """测试添加 Socket 类型设备"""
        device_id = "test-device-002"
        mock_connection = Mock()

        await manager.add_device(device_id, mock_connection, "socket")

        assert manager.connected_devices[device_id]["type"] == "socket"

    @pytest.mark.asyncio
    async def test_remove_device_existing(self, manager):
        """测试移除存在的设备"""
        device_id = "test-device-001"
        await manager.add_device(device_id, Mock(), "websocket")
    ...
    @pytest.mark.asyncio
    async def test_remove_device_non_existing(self, manager):
        """测试移除不存在的设备"""
        # 不应该抛出异常
        await manager.remove_device("non-existing-device")

    @pytest.mark.asyncio
    async def test_is_device_connected_true(self, manager):
        """测试检查设备已连接"""
        device_id = "test-device-001"
        await manager.add_device(device_id, Mock(), "websocket")

        assert await manager.is_device_connected(device_id) is True

    @pytest.mark.asyncio
    async def test_is_device_connected_false(self, manager):
        """测试检查设备未连接"""
        assert await manager.is_device_connected("non-existing") is False

    @pytest.mark.asyncio
    async def test_get_device_existing(self, manager):
        """测试获取存在的设备"""
        device_id = "test-device-001"
        mock_conn = Mock()
        await manager.add_device(device_id, mock_conn, "websocket")

        result = await manager.get_device(device_id)

        assert result is not None
        assert result["connection"] == mock_conn

    @pytest.mark.asyncio
    async def test_get_device_non_existing(self, manager):
        """测试获取不存在的设备"""
        result = await manager.get_device("non-existing")

        assert result is None

    @pytest.mark.asyncio
    async def test_get_all_devices(self, manager):
        """测试获取所有设备"""
        await manager.add_device("dev-001", Mock(), "websocket")
        await manager.add_device("dev-002", Mock(), "socket")
    ...
    async def test_remove_console(self, manager):
        ...
        device_id, session_ids = await manager.remove_console(mock_websocket)
