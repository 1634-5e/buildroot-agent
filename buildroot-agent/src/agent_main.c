...
int agent_init(const char *config_path, const config_override_t *overrides)
{
    ...
    /* 初始化 Device Twin 模块 */
    if (g_agent_ctx->config.enable_twin) {
        if (agent_twin_init(g_agent_ctx) != 0) {
            LOG_WARN("Device Twin 模块初始化失败");
        }
    }
...
int agent_start(void)
{
    ...
    /* 启动 Device Twin 线程 */
    if (g_agent_ctx->config.enable_twin && g_agent_ctx->twin_sync) {
        if (agent_twin_start(g_agent_ctx) == 0) {
            LOG_INFO("Device Twin 线程启动");
        }
    }
...
void agent_cleanup(void)
{
    ...
    /* 清理 Device Twin */
    if (g_agent_ctx->config.enable_twin) {
        agent_twin_stop(g_agent_ctx);
        agent_twin_cleanup(g_agent_ctx);
    }
...
static void print_help(const char *prog)
{
    ...
    printf("用法: %s [选项]\n\n", prog);
    printf("选项:\n");
    printf("  -c, --config <path>   指定配置文件路径 (默认: <exe_dir>/agent.cfg)\n");
    ...
    printf("\n");
    printf("示例:\n");
    printf("  %s -c /etc/agent.cfg -d\n", prog);
    printf("  %s -g -e -c agent.cfg.example\n", prog);
}
