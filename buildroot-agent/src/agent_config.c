...
void config_set_defaults(agent_config_t *config)
{
    ...
    /* Device Twin (MQTT) 配置 */
    config->enable_twin = false;
    set_string_field(config->mqtt_broker, sizeof(config->mqtt_broker), "localhost");
    config->mqtt_port = 1883;
    config->twin_report_interval = 30;
}
...
void config_load_from_env(agent_config_t *config)
{
    ...
    /* Device Twin (MQTT) 配置 */
    val = getenv("BUILDROOT_ENABLE_TWIN");
    if (val) {
        config->enable_twin = (strcmp(val, "true") == 0 || strcmp(val, "1") == 0);
    }
    
    val = getenv("BUILDROOT_MQTT_BROKER");
    if (val) {
        set_string_field(config->mqtt_broker, sizeof(config->mqtt_broker), val);
    }
    
    val = getenv("BUILDROOT_MQTT_PORT");
    if (val) {
        int port = atoi(val);
        if (port > 0) config->mqtt_port = port;
    }
    
    val = getenv("BUILDROOT_MQTT_USERNAME");
    if (val) {
        set_string_field(config->mqtt_username, sizeof(config->mqtt_username), val);
    }
    
    val = getenv("BUILDROOT_MQTT_PASSWORD");
    if (val) {
        set_string_field(config->mqtt_password, sizeof(config->mqtt_password), val);
    }
}
...
static int parse_config_line(agent_config_t *config, const char *key, const char *value)
{
    ...
    } else if (strcmp(key, "enable_twin") == 0) {
        config->enable_twin = (strcmp(value, "true") == 0 || strcmp(value, "1") == 0);
    } else if (strcmp(key, "mqtt_broker") == 0) {
        set_string_field(config->mqtt_broker, sizeof(config->mqtt_broker), value);
    } else if (strcmp(key, "mqtt_port") == 0) {
        config->mqtt_port = atoi(value);
    } else if (strcmp(key, "mqtt_username") == 0) {
        set_string_field(config->mqtt_username, sizeof(config->mqtt_username), value);
    } else if (strcmp(key, "mqtt_password") == 0) {
        set_string_field(config->mqtt_password, sizeof(config->mqtt_password), value);
    } else if (strcmp(key, "twin_report_interval") == 0) {
        config->twin_report_interval = atoi(value);
    } else {
        return -1;
    }
...
void config_print(agent_config_t *config)
{
    ...
    LOG_INFO("Device Twin: %s", config->enable_twin ? "启用" : "禁用");
