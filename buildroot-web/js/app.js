// ============================================
// Application Logic - Device Management, Files, Monitor
// ============================================
...
function navigateTo(path) {
    currentPath = path;
    // terminalPath element doesn't exist in current HTML - commented out
...
function sendDownloadRequest(path) {
    sendMessage(MSG_TYPES.DOWNLOAD_PACKAGE, {
        path: path,
        format: 'tar'
    });
...
function displayEditorContent(content, readonly = false, retryCount = 0) {
    const preview = document.getElementById('filePreview');
...
function onFileSaveError(error) {
    pendingFileSave = null;
...
function showFileConflictDialog(error) {
...
function confirmFileSave(force) {
    const overlay = document.getElementById('fileConflictDialog');
    if (overlay) {
        overlay.style.display = 'none';
    }
